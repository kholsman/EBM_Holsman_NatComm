...
\begin{Highlighting}[]
    ...
    \NormalTok{url <-}\StringTok{   "https://ndownloader.figshare.com/files/23756117?private_link=81007e2dd5edee0a5a7a"}
...
Holsman, K. K., A. Haynie, A. Hollowed, J.C.P. Reum, K. Aydin, A. J.
Hermann, W. Cheng, A. Faig, J. N. Ianelli, K. Kearney, A. E.
Punt,2020.The Alaska Climate Integrated Modeling study Multispecies
Ecosystem Based Fisheries Management and climate change simulation and
results data. DOI: 10.6084/m9.figshare.12568625

as well as the corresponding manuscript:

Holsman, K. K., A. Haynie, A. Hollowed, J.C.P. Reum, K. Aydin, A. J.
Hermann, W. Cheng, A. Faig, J. N. Ianelli, K. Kearney, A. E. Punt,
2020.Ecosystem-based fisheries management forestalls climate-driven
collapse, Nature Communications.
...
\begin{Highlighting}[]
    ...
    \KeywordTok{source}\NormalTok{(}\StringTok{"R/sub_scripts/make_plots.R"}\NormalTok{)  }\CommentTok{# this throws warnings that can be ignored}
...
\begin{Highlighting}[]
 ...
 \NormalTok{tmp <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\StringTok{"No Cap"} \NormalTok{=}\StringTok{ }\KeywordTok{c}\NormalTok{( }
 ...
 \CommentTok{# get mean and var for tipping points:}
 \KeywordTok{mean}\NormalTok{(}\KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{unlist}\NormalTok{(tmp)))}
 \KeywordTok{sd}\NormalTok{(}\KeywordTok{as.numeric}\NormalTok{(}\KeywordTok{unlist}\NormalTok{(tmp)))}
   
 \CommentTok{# double check:}
 \NormalTok{threshIN     <-}\StringTok{ }\NormalTok{C_thresh_13_2}
 \NormalTok{thrsh2_all   <-}\StringTok{ }\KeywordTok{intersect}\NormalTok{(threshIN$signif2,threshIN$ix_pks)}
 \NormalTok{df2_qnt      <-}\StringTok{ }\NormalTok{threshIN$df2_qnt}
 \NormalTok{df2_qnt$tmp[(thrsh2_all[}\KeywordTok{which}\NormalTok{( }\KeywordTok{abs}\NormalTok{(df2_qnt$smoothed_mn[thrsh2_all])  ==}\StringTok{ }\KeywordTok{max}\NormalTok{(}\KeywordTok{abs}\NormalTok{(df2_qnt$smoothed_mn[thrsh2_all])) ) ] ) ]}
 \NormalTok{df2_qnt$tmp[(thrsh2_all[}\KeywordTok{which}\NormalTok{( }\KeywordTok{abs}\NormalTok{(df2_qnt$smoothed_dwn[thrsh2_all]) ==}\StringTok{ }\KeywordTok{max}\NormalTok{(}\KeywordTok{abs}\NormalTok{(df2_qnt$smoothed_dwn[thrsh2_all])) )])  ]}
 \NormalTok{df2_qnt$tmp[(thrsh2_all[}\KeywordTok{which}\NormalTok{( }\KeywordTok{abs}\NormalTok{(df2_qnt$smoothed_up[thrsh2_all])  ==}\StringTok{ }\KeywordTok{max}\NormalTok{(}\KeywordTok{abs}\NormalTok{(df2_qnt$smoothed_up[thrsh2_all]))  )])  ]}
\end{Highlighting}
\end{Shaded}
