...
############################################################################################
	...
	# rm(list=ls()); setwd("/Users/kholsman/GitHub/CEATTLE/runs/assmnt_2018_0")
...
############################################################################################
	...
	############################################################################################
		...
		plot_rec(logg=FALSE,ylimm=NA,coll=c(col0(1),col2(1)),asmt=F,asmt_dat="")
		if(update.figs==1){
	...
	##  Mortality & predation
	############################################################################################
		plot_M2(coll=c(colors()[330],"black"),ltyy=c(2,1),ylimm=NA)
		if(update.figs==1){
		...
		calc_eaten<-function(dat1=ceattle_2,pred=2,prey=1){
			...
			eval(parse(text=paste("M1<-dat1$M1_",prey,sep="")))
			eval(parse(text=paste("M2<-dat1$M2_",prey,sep="")))
			eval(parse(text=paste("F<-dat1$F_",prey,sep="")))
			eval(parse(text=paste("Bage<-dat1$biomassByage_",prey,sep="")))
			eval(parse(text=paste("Nage<-dat1$Nage_",prey,sep="")))
			...
			Mtmp  <-
			prey_consumed_all<-
			prey_consumed<- matrix(0,nyrs,nages_prey)
			...
			for( i in 1:nyrs){
				...
				for(pred_age in 1:nages_pred){
			        ...
			        Etmp[i,]         = Etmp[i,]+((Pred_demand[i,pred_age]/1000)*suit_main[prey,1:nages_prey])
			        Mtmp[i,]         = Mtmp[i,]+((Pred_demand[i,pred_age]/1000)*suit_main[prey,1:nages_prey])/(avail_food[i,pred_age])
			        
			    }  # end pred sp_age loo
			    prey_consumed[i,] =  (wt[i,] *(((Mtmp[i,]/Z[i,]) * ((1-exp(-Z[i,]))* Nage[i,])  )) )
			    prey_consumed_all[i,] =  (wt[i,] *(((M2[i,]/Z[i,]) * ((1-exp(-Z[i,]))* Nage[i,])  )) )

		    } 
		    return(list(Pred_demand=Pred_demand,Etmp=Etmp,Mtmp=Mtmp,avail_food=avail_food,eaten=eaten,prey_consumed=prey_consumed,prey_consumed_all=prey_consumed_all))
		}	
	...
	############################################################################################
		...
		Dat4Liz<-list(plkEatingPlk=tmp1_1$prey_consumed,PcodEatingPlk=tmp2_1$prey_consumed,ATFEatingPlk=tmp3_1$prey_consumed,totalPlkEaten=ceattle_2$PLK_consumed,years=years)
		...
		prey1_eaten<-apply(tmp1_1$Etmp,1,sum)+apply(tmp2_1$Etmp,1,sum)+apply(tmp3_1$Etmp,1,sum)
		...
		plot_indiv_ration<-function(dat=ceattle_2,ylimm_all=rbind(c(150,280),c(200,8500),c(200,3000))/1000 ){
			...
			layout(c(1,2,3))
			layout.show(3)
			xatt<-seq(1960,2050,5)
			spnames<-c("a) Walleye pollock","b) Pacific cod","c) Arrowtooth flounder")
			for(s in 1:3){
			# eval(parse(text=paste0("ddtt<-rowSums(E_est_all_",s,")/1e9")))

			if(s==1) {ddtt<-tmp$Consum_1/1000;bbtt<-tmp$biomassByage_1 }
			if(s==2) {ddtt<-tmp$Consum_2/1000;bbtt<-tmp$biomassByage_2 }
			if(s==3) {ddtt<-tmp$Consum_3/1000;bbtt<-tmp$biomassByage_3 }
			ddtt<-apply( (ddtt*bbtt)[,-(1:3)]/apply(bbtt[,-(1:3)],1,sum),1,sum)
			tmp$biomassByage_1
			yy<-1979+(1:nyrs-1)
			scatter.smooth(yy, ddtt,type="b",pch=16,span =10/nyrs,
			       ylab="",xlab="",axes=F,	ylim=ylimm_all[s,],
			       lpars =list(col = "orange", lwd = 2, lty = 1))
			#plot(1979+(1:nyrs-1),(ddtt),type="b",pch=16,ylab="",xlab="",axes=F)	
			if(s==2) mtext("Annual ration (kg/individual)",side=2,line=3,outer=FALSE,font=2)
			mtext(side=3,line=-1.5,adj=.01,spnames[s],font=T,outer=F)
			abline(h=mean((ddtt)),lty=1,col="gray")
			abline(h=mean((ddtt))+sd(ddtt),lty=2,col="gray")
			abline(h=mean((ddtt))-sd(ddtt),lty=2,col="gray")
			points(1979+(1:nyrs-1),(ddtt),type="b",pch=16)
			axis(1,at=xatt,lab=rep("",length(xatt)));axis(2,las=2)
			if(s==3) axis(1,at=xatt)
			axis(1,at=c(1900,2100));axis(2,at=c(-100,100))
			}

			mtext(outer=T, "Annual ration (adult; age 4 +)",font=2,line=.5)
		}
		plot_indiv_ration()
		if(update.figs==1) quartz.save(file=file.path(plot_fileJPG,"annual_ration_index.pdf"),type="pdf",dpi=500) #(dev.copy2eps(device = quartz, file =file.path(plot_file,"plot_C.eps")))
