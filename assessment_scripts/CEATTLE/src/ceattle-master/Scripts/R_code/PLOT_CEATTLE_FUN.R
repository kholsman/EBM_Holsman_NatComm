...
plot_rec<-function(ltyy=c(2,1), asmt_dat=asmt_dat1,asmt=TRUE,coll=collrange1(2),lwdd=c(2,2),err_bar=FALSE,logg=TRUE,ylimm=c(1e12,20e9,10e9)){
	...
	for(sp in 1:nspp){
		...
		obs2<-exp(ceattle_2$logR_obs[sp,])*1000 # N is in thousands of fish

		nn<-length(years[-1])
		cc<-.1/1000
		if(logg==TRUE){
			if(is.na(ylimm[1])){ ylimmIN<-1.1*max(log(c(obs,obs2)),na.rm=T)}else{ylimmIN<-ylimm[sp]}
			plot(log(obs)~years,pch=16,type="p",ylim=c(1,ylimmIN),main="",axes=FALSE,lwd=1,cex=1.2,col="white")
		}else{
			if(is.na(ylimm[1])){ ylimmIN<-1.1*max((c(obs,obs2)),na.rm=T)}else{ylimmIN<-ylimm[sp]}
			plot((obs)~years,pch=16,type="p",ylim=c(0,ylimmIN),main="",axes=FALSE,lwd=1,cex=1.2,col="white")
		}
		...
		axis(1)
		if(logg==TRUE){
			att<-(c(100,1e4,1e6,pretty(seq(0,exp(ylimmIN),1e6),4)))
			...
			att<-(pretty(seq(0,ylimmIN,1e6),4))
		...
		text(1979,ylimmIN*.95,spnames[sp],pos=4,font=2,cex=1.2)
...
plot_M2<-function(ltyy=c(1,2),coll=collrange1(2),lwdd=c(2,2),ylimm=c(2.5,1,1)){
	quartz(height=6,width=4)
	par(mar=c(2,1,0,0)) # margins of graph: (bottom,left, top, right)
	par(mgp=c(2,.5,0)) # axis margins: (distance of lab, distance of numbers, axis line)
	par(oma=c(2,3.5,1,1))# outer margins of graph: (bottom,left, top, right)
	...
	for(s in 1:nspp)
	{
		...
		if(is.na(ylimm[1])){ylimmIN<-1.1*max(dat2[,1]+dat2_1[1,1],na.rm=T)}else{ylimmIN<-ylimm[s]}
		plot(years,dat2[,1]+dat2_1[1,1],type="p",axes=FALSE,col="white",lwd=1,lty=ltyy[2],pch=19,cex=1.2,ylim=c(0,ylimmIN))
		yatt<-pretty(seq(0,ylimmIN,.1),4)
		axis(2,at=yatt,lab=yatt,las=2)
		...
		text(1979,ylimmIN*.95,spnames[s],pos=4,font=2,cex=1.2)
		if(s<3)
			axis(1,at=xatt,lab=rep("",length(xatt)),font=1)
		if(s==3)
			axis(1,at=xatt,font=1)
		if(s==2)
			legend("bottomleft",c("single-species", "multi-species"),col=coll,lwd=lwdd,lty=ltyy,box.lty=0,cex=1.2)
	}
