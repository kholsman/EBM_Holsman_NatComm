...
calcDelta<-function(datIN       = datuse_B,
  ...
  names(datIN)[names(datIN)==delta_var_nm] <- "var"
      ...
      for(y in 1:(nbin-1)){
        ...
        dd_ref             <-  datIN%>%filter(Year>YrbinIN[y]&Year<=YrbinIN[y+1],Scenario==ref_scen)%>%select("sp","MC_n",Scenario_ref=Scenario,"Year",TempC_ref=bottomT_C,var_ref=var)
        ...
        dd_delta$rcp       <-  names(esmlistIN)[1]
        dd_delta$rcp[dd$Scenario%in%esmlistIN[[2]]]       <-  names(esmlistIN)[2]
