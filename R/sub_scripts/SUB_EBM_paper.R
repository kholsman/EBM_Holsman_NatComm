...
## ------------------------------------------------
        ...
        sub <- sim_msm%>%group_by(harvestMode,Scenario,species,age,future_year,bottomT_C,
                           wt_at_age,ABC_total_biom ,Catch_total_biom, Catch_Num_byAge,
                           pmature,B,B0 ,      SSB   ,   SSB0,
                           SSB_total_biom, SSB0_total_biom,N,N0,M1,M2, F40, F35, Fabc, F,
                           F_age, Ration_g_fish ,MC_n, ID ,mode, recMode, hMode, hModev2, note)%>%
        ...
        rset            <-  c(1,5)[2]   # use climate enhanced recruitment
            ...
            dat <- rbind(dat_2_5_12, dat_2_5_13)
            dat <- dat_2_5_12
            dat <- dat_2_5_13
            
             dat2       <- as_tibble(dat)%>%filter(age==1)
            
            ggplot(data=dat2) +
             geom_line(aes(x=future_year,y=Catch_total_biom,color=as.factor(Scenario))) +
             facet_grid(factor(species)~factor(hModev2),scales="free_y")
